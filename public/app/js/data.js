...
const _cache = { budgets: [], revenues: [], expenses: [], payables: [], receivables: [], funds: [], procurement: [], assets: [], journalEntries: [] };
/* -------------------------------- budgets -------------------------------- */
...
let _glAccountsCache = null;

async function getGLAccounts() {
  if (!_glAccountsCache) {
    const res = await _api('GET', '/api/general-ledger/accounts');
    _glAccountsCache = res.data;
  }
  return _glAccountsCache;
}

async function getJournalEntries(filters) {
  const params = new URLSearchParams();
  if (filters && filters.date_from) params.set('date_from', filters.date_from);
  if (filters && filters.date_to) params.set('date_to', filters.date_to);
  const query = params.toString();
  const res = await _api('GET', '/api/general-ledger/entries' + (query ? '?' + query : ''));
  _cache.journalEntries = res.data;
  return res.data;
}
function getJournalEntry(id) { return _cache.journalEntries.find(e => e.id === id) || null; }

async function createJournalEntry(data) {
  const res = await _api('POST', '/api/general-ledger/entries', {
...
async function reverseJournalEntry(id) {
  ...
  const res = await _api('POST', `/api/general-ledger/entries/${raw}/reverse`);
  return res.data;  
}
